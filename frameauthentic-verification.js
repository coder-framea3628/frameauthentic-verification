...
#confirmSim:hover, #confirmNao:hover {
...
#botaoEntendi, #botaoRetry, #startButton {
  background: var(--accent-color);
  color: white;
  padding: 10px 16px;
  border: none;
...
#botaoEntendi:hover, #botaoRetry:hover, #startButton:hover {
  background: var(--accent-light);
  transform: scale(1.02);
}

#botaoRetry:disabled, #startButton:disabled {
  background: #ccc;
  cursor: not-allowed;
}
...
<div id="popup-overlay" style="display: none;">
  <div id="popup-box">
    ...
    <div id="camera-container">
      ...
      <div id="face-circle"></div>
      <img id="watermark" src="https://framerusercontent.com/images/Oe8d7JZskbAvjlTrwbIEeozgs.png" alt="Frame Authentic Logo">
      ...
      <div id="falha-overlay">
        ...
        <button id="botaoEntendi">Certo, voltar ao cadastro</button>
      </div>
    </div>
    ...
    <button id="startButton" style="display: none;">Iniciar Verificação</button>
  </div>
</div>
<div id="confirm-overlay" style="display: none;">
  <div id="confirm-box">
    <h3>Confirmar Retorno</h3>
    <p>Deseja voltar ao cadastro?</p>
...
const strings = {
  ...
  permissionDenied: 'Permissão a câmera negada',
  permissionDeniedBottom: 'Reinicie a página para tentar novamente',
  attemptsRemaining: 'Tentativas restantes: ',
  blockedTop: 'Acesso Bloqueado',
  blockedBottom: 'O sistema Frame Authentic está bloqueado por 24 horas devido a tentativas inválidas.',
  blockedCountdown: 'Tempo restante: ',
  errorReasons: [
    'Falha na detecção biométrica.',
    'Erro de processamento de imagem no servidor.',
    'Instabilidade na sua conexão de verificação.',
    'Qualidade de captura abaixo do padrão requerido.',
    'Problema técnico na sua análise facial.'
  ]
};
...
function isBlocked() {
  const now = Date.now();
  return now < blockEndTime;
}

function chooseErrorReason() {
  const index = Math.floor(Math.random() * strings.errorReasons.length);
  currentErrorReason = strings.errorReasons[index];
  localStorage.currentErrorReason = currentErrorReason;
}
...
function startCountdown() {
  ...
  function updateCountdown() {
    ...
    if (remainingMs <= 0) {
      ...
      attempts = 0;
      localStorage.removeItem('currentErrorReason');
      currentErrorReason = '';
      initVerification();
    } else {
      const hours = Math.floor(remainingMs / (60 * 60 * 1000));
      const minutes = Math.floor((remainingMs % (60 * 60 * 1000)) / (60 * 1000));
      attemptsCountdown.textContent = `${strings.blockedCountdown}${hours} horas e ${minutes} minutos`;
    }
  }
}
...
function showBlockScreen() {
  ...
  statusTop.textContent = strings.blockedTop;
  statusBottom.textContent = strings.blockedBottom;
  instructions.style.display = 'none';
  progressBar.style.display = 'none';
  cameraContainer.style.display = 'none';
  botaoRetry.style.display = 'none';
...
function showStartScreen() {
  ...
  botaoRetry.style.display = 'none';
  botaoEntendi.style.display = 'none';
  statusTop.textContent = 'Preparando Verificação';
  statusBottom.textContent = 'Siga as instruções acima e clique em Iniciar.';
  attemptsCountdown.textContent = attempts > 0 ? `${strings.attemptsRemaining}${maxAttempts - attempts}` : '';
...
function initCamera() {
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } })
    .then(function(s) {
      ...
      // Simula processamento com mais passos
      setTimeout(() => { progress.style.width = '30%'; }, 1000);
      setTimeout(() => { progress.style.width = '60%'; }, 3000);
      setTimeout(() => { progress.style.width = '90%'; }, 5000);

      // Sempre falha após tempo randômico (6-10s)
      const failDelay = Math.random() * 4000 + 6000;
...
function handleFailure() {
  ...
  falhaReason.textContent = currentErrorReason;
  ...
  logAction(`Falha na verificação: ${currentErrorReason} (Tentativa ${attempts})`);
  watermark.style.display = 'none';
...
function showFailButtons() {
  ...
  botaoEntendi.style.display = 'inline-block';
}
...
function initVerification() {
  ...
  popupOverlay.style.display = 'flex';
  if (attempts === 0 || !currentErrorReason) {
    chooseErrorReason();
  }
  showStartScreen();
}
