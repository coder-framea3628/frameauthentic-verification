...
Após o envio, nossa tecnologia realizará a análise dos dados biométricos.</div>
    ...
    async function checkBlockStatus() {
        ...
        if (blockEnd && Date.now() < parseInt(blockEnd)) {
            ...
            if (level === '2') {
                text = "Seu acesso foi restrito por 3 dias devido a falhas consecutivas na verificação. Caso novas tentativas inválidas ocorram, o acesso ao Frame Authentic poderá ser bloqueado permanentemente. Recomendamos revisar atentamente as instruções e tentar novamente após o término do período.";
            }
    ...
    function takePicture() {
        const context = elements.canvas.getContext('');
    ...
    async function handleFail() {
        ...
        if (remaining <= 0) {
            ...
            // Erros na validação
            const reasons = ["Iluminação insuficiente", "Rosto não centralizado", "Movimentos liveness incorretos", "Baixa qualidade de imagem"];
